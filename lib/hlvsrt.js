...
`;
	...
	contents += this.subtitles.map(x => {
		...
		return `Dialogue: ${x.style}, ${x.getStart()}, ${x.getEnd()}, ${text}`
	}).join("\n");
	...
	const churn = start => {
		...
		this.translate(text, (e, tr) => {
			...
			exports.readWhole(tr, (e, b) => {
				...
				let chunks = JSON.parse(b.toString("utf-8")).split(sep);
				let ns;
				...
				for(let i = 0; i < chunks.length; i++) {
					/* 
					   Sometimes translator generates extra chunks at the end 
...
Tool.prototype.run = function() {
	...
	if (this.options.merge) {
		...
		const tp = this.options.args[1] || bp.replace(/[.]srt$/i, ".hugo.srt");
	...
	if (skipped)
		console.error("Skipping %j paths that were already processed");
...
Tool.prototype.translateFiles = function() {
	...
	Translator(key, options).
		translateSubtitlesFile(sf, this, (e, file) => {
			if (e)
				throw e;
			file.path = sf.path.replace(/[.]srt$/i, ".hugo.srt");
			file.save(e => {
				if (e)
					throw e;
				this.printStatus(util.format("Written %j\n", file.path));
				const ssa = SsaFile();
				ssa.path = sf.path.replace(/[.]srt$/i, ".hugo.ass");

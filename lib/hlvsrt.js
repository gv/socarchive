...
UnzipList.prototype.getAllPaths = function(cb) {
	...
	const r = zips.false, cycle = paths => {
		...
		const cwd = this.outdir || path.dirname(p);
		const cmd =
			  ["unzip", "-oC", path.resolve(p)].concat(this.masks);
...
SrtFile.prototype.load = function(cb) {
	...
	fs.stat(this.path, (e, st) => {
		...
		this.loadExstingFile(cb);
	});
	return this;
};
	
SrtFile.prototype.loadExstingFile = function(cb) {		   
...
SsaFile.prototype.save = function(cb) {
	...
	const style = 0 ? {alignment:9} : {alignment:2};
	let contents = `\
[Script Info]
ScriptType: v4.00+
PlayResX: 384
...
Style: Bottom,Arial,6,&Hffffff,&Hffffff,&H0,&H0,0,0,0,0,100,100,0,0,1,1,0,${style.alignment},10,10,10,0
...
Translator.prototype.translateStrings = function(ss, fpath, tool, cb) {
	const cl = 16000, sep = "23985623865";
...
Tool.prototype.run = function(cb) {
	...
	if (this.options.merge) {
		...
		const tp = this.options.args[1] || this.outPath(bp, 0);
		const bottom = SrtFile(bp, this.options).load(e => {
			...
			const top = SrtFile(tp, this.options).load(e => {
				if (e)
					throw e;
				const r = SsaFile(this.outPath(bp, 1));
	...
	list.masks = ["*.srt"];
	list.outdir = this.options.outdir;
	list.getAllPaths((e, paths) => {
		...
		this.files = [];
		this.processTheRest(e => {
			// Zips must be deleted only on success, so in case of
			// failure the exact command line can be rerun
			if (e)
				throw e;
...
Tool.prototype.processTheRest = function(cb) {
	const path = this.paths.shift();
	if (!path) {
		this.translateFiles(cb);
		return;
	}
	...
	f.load((e, sf) => {
		...
		process.nextTick(() => this.processTheRest(cb));
	});
};

Tool.prototype.outPath = function(inputPath, index) {
	const dir = this.options.outdir || path.dirname(inputPath);
	if (index === 0) {
		inputPath = "." + path.basename(inputPath);
	}
	return path.join(dir, path.basename(inputPath.replace(
		/[.][^.]+$/i, [".hugo.srt", ".hugo.ass"][index])));
};

Tool.prototype.translateFiles = function(cb) {

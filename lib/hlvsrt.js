...
const SrtFile = function(path, encoding) {
	if (!new.target)
		return new SrtFile(path, encoding);
	this.path = path;
...
Translator.prototype.translateStrings = function(ss, fpath, tool, cb) {
	const cl = 13000, sep = "23985623865";
...
Tool.prototype.run = function(cb) {
	...
	if (this.options.merge) {
		...
		const bottom = SrtFile(bp, this.options.ie).load(e => {
			if (e)
				throw e;
			const top = SrtFile(tp, this.options.ie).load(e => {
				if (e)
					throw e;
